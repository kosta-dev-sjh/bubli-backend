...
### 작업 카드 AB-2. UserPublicService 공개 계약 분리

처리 시각: 2026-06-25 18:55 KST

브랜치: `codex/abstraction-boundary-user-public-service`

변경 내용:

- #29 `chore/latest-docs-2026-06-25` 기준으로 `UserPublicService`를 인터페이스 계약으로 분리했다.
- 기존 사용자 조회/존재 확인 구현은 `UserPublicServiceImpl`로 옮겼다.
- `project`와 `chat` 도메인은 기존 이름인 `UserPublicService` 인터페이스를 계속 주입받는다.
...
- `git diff --check` 통과
- PR #87 head `eec0a51` 기준 GitHub checks 없음. 현재 CI workflow의 `pull_request.branches`가 `develop`, `main`, `feature/**`만 포함하고, PR #87 base가 `chore/latest-docs-2026-06-25`라서 check run이 생성되지 않는다.

후속 PR stack 반영 순서:

1. #29 문서 PR 뒤에 이 보정 PR을 반영한다.
2. `UserPublicService`를 참조하는 #48~#51 user/settings stack과 chat/project 계열 PR은 이 PR 반영 뒤 최신 base를 병합하거나 필요한 최소 충돌만 해결한다.
3. 후속 PR에서 구현체를 직접 주입하는 코드가 생기면 `UserPublicService` 인터페이스 주입으로 되돌린다.
