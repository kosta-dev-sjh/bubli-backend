...
Last checked: 2026-06-24 21:39:10 KST
...
| 현재 확인 브랜치 | `feature/agent-storage-foundation` |
| 원격 기준 브랜치 | `develop` |
...
- `./gradlew compileTestJava` 통과 (2026-06-24 21:36 KST)
- `./gradlew cleanTest test` 통과 (2026-06-24 21:37 KST)
- `git diff --check` 통과 (2026-06-24 21:37 KST)
...
| #24 | `chore: Google-only 인증 기반 정리` | `feature/auth-google-foundation` | `223e01d` | #23 기준 draft stacked PR 생성됨. GitHub Actions checks 없음. #22, #23 merge 후 `develop` 기준 CI 재확인 필요 |
| #25 | `feat: 자료 기본 저장 조회 API 추가` | `feature/resource-basic-foundation` | `d69a5ad` | #24 기준 draft stacked PR 생성됨. GitHub Actions checks 없음. #22, #23, #24 merge 후 `develop` 기준 CI 재확인 필요 |
| #26 | `feat: 에이전트 저장 기반 추가` | `feature/agent-storage-foundation` | `8bed946` | #25 기준 draft stacked PR 생성됨. GitHub Actions checks 없음. #22, #23, #24, #25 merge 후 `develop` 기준 CI 재확인 필요 |
...
| 인증 | `POST /api/auth/signup` 포함 | 최종 기획문서와 백엔드 가이드는 Google-only 인증을 기준으로 하므로 코드에서는 signup endpoint와 email/password 흔적을 제거함. 최종 API 수정본에서 auth endpoint 표기 보정 필요 |
...
| 에이전트 후보 타입 | 기획/가이드는 TODO, WBS, REQUIREMENT, SCHEDULE, QUESTION, CONTRACT_FIELD, CONTRACT_REVIEW, DOCUMENT_DRAFT, DAILY_SUMMARY, MEMO 등 후보를 통합 저장한다고 설명 | 현재 코드 enum은 `WBS`, `TASK`, `REQUIREMENT`, `SCHEDULE`, `REVIEW_ITEM`, `QUESTION` 중심이다. 최종 API 기준 후보 타입 정리가 필요 |
...
6. 추천 다음 작업은 Entity/Flyway 정합성 점검이다. 특히 `agent_model_call_logs`, `agent_suggestions`, `resource` 관련 테이블을 먼저 본다.
7. #19~#26은 현재 API 기준으로 계속 진행하되, 수정본 API가 오면 차이 보정한다.
