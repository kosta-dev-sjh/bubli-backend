...
Last checked: 2026-06-25 17:39 KST
...
| #39 | `[feat] 저장 용량 조회 API 추가` | `feature/storage-usage-api` | `feature/resource-related-api` | `53e1fb0` | checks 없음, merge dirty, draft | 최신 #31 head `3bf14f5` 기준 충돌 해소와 로컬 검증 후 ready 전환 |
...
7. #39는 `RoomMemberRepository.java` 충돌 보류 상태다. 재개 시 최신 #31 head `3bf14f5`와 #39 양쪽 메서드를 보존하는 수동 병합이 필요하다.
8. #67 이후 resource delete stack은 #67 최신 head 기준으로 순차 재검증한다.
9. 남은 draft PR은 #39, #62이며 각각 DIRTY 해소 뒤 ready 전환한다.
