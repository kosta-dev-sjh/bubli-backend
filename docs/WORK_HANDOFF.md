...
Last checked: 2026-06-25 03:55 KST
...
| 현재 확인 브랜치 | `feature/analyze-resource-job-api` |
| 원격 기준 브랜치 | `develop` |
...
- `./gradlew compileTestJava` 통과 (2026-06-25 03:27 KST, room ai-documents API)
- `./gradlew cleanTest test` 통과 (2026-06-25 03:27 KST, room ai-documents API)
- `git diff --check` 통과 (2026-06-25 03:27 KST, room ai-documents API)
- `./gradlew compileTestJava` 통과 (2026-06-25 03:55 KST, analyze-resource job API)
- `./gradlew cleanTest test` 통과 (2026-06-25 03:55 KST, analyze-resource job API)
- `git diff --check` 통과 (2026-06-25 03:55 KST, analyze-resource job API)
- 엔티티 44개, Repository 4개, Controller 4개, Service 5개 확인
- 현재 API 기준 세부 작업 지시는 `docs/CURRENT_API_BASELINE_WORK.md`를 기준으로 나눈다.

## 최근 완료 작업

### 작업 카드 7-8. analyze-resource 에이전트 작업 생성 API

처리 시각: 2026-06-25 03:55 KST

변경 내용:

- `feature/analyze-resource-job-api`를 #37 `feature/room-ai-documents-api` 위의 새 브랜치로 만들었다.
- 현재 `10_API-Design.md` 기준 `POST /api/ai/analyze-resource`를 추가했다.
- `ResourceService.getResource`로 자료 읽기 권한을 먼저 확인한 뒤 `ANALYZE_RESOURCE` agent job을 `PENDING`으로 생성한다.
- 실제 모델 실행, 요약/임베딩/AI 문서 생성, 후보 생성은 아직 실행하지 않는다.
- `AiJobCommandService`를 추가해 resource 접근 확인과 agent job 생성을 묶었다.
- `AnalyzeResourceRequest` DTO를 추가하고, 응답은 `AgentJobResponse` DTO를 사용한다.
...
- `git diff --check` 통과
- GitHub Actions checks 없음. #40은 #37 위의 stacked PR이라 현재 base 체인에서는 workflow가 실행되지 않았다. base 체인 merge 후 `develop` 기준 CI 재확인 필요.

메모:

- 이번 PR은 자료 분석 작업 생성만 다룬다.
...
### 작업 카드 7-7. project-room ai-documents 목록 조회 API

처리 시각: 2026-06-25 03:27 KST

변경 내용:

- `feature/room-ai-documents-api`를 #36 `feature/resource-ai-document-api` 위의 새 브랜치로 만들었다.
- 현재 `10_API-Design.md` 기준 `GET /api/project-rooms/{roomId}/ai-documents`를 추가했다.
- 프로젝트룸 AI 문서 목록은 `ProjectMembershipPublicService.assertActiveMember`로 ACTIVE 멤버만 조회할 수 있게 했다.
...
- `./gradlew cleanTest test` 통과
- `git diff --check` 통과
- GitHub Actions checks 없음. #37은 #36 위의 stacked PR이라 현재 base 체인에서는 workflow가 실행되지 않았다. base 체인 merge 후 `develop` 기준 CI 재확인 필요.

메모:

- 이번 PR은 프로젝트룸 AI 문서 목록 조회만 다룬다.
- AI 문서 생성/분석 실행, resource download-url/S3 presigned URL은 후속 PR로 남긴다.
- 이번 변경에는 Gradle, GitHub Actions, PR 템플릿, README, SETUP 같은 초기 개발환경 세팅 파일 변경이 없다.
...
- 2026-06-25 20:58 KST에 #92의 `ProjectMembershipPublicService`, `UserPublicService`, `TaskPublicService`, `WbsItemPublicService` 공개 계약 분리 커밋도 이 stack에 cherry-pick했다.
...
- `./gradlew cleanTest test` 통과
- `git diff --check` 통과
- PR #90 head `abd042b` 기준 GitHub checks 없음. #36 base `feature/resource-ai-document-api`의 CI workflow가 `pull_request.branches`를 `develop`, `main`만 포함하도록 설정해 check run이 생성되지 않는다.
...
| #36 | `[feat] 자료 AI 문서 조회 API 추가` | `feature/resource-ai-document-api` | `d8bea2b` | #35 기준 draft stacked PR 생성 완료. 로컬 검증 통과. GitHub Actions checks 없음. base #35에는 #28의 stacked PR CI workflow가 아직 포함되지 않음 |
| #37 | `[feat] 프로젝트룸 AI 문서 목록 조회 API 추가` | `feature/room-ai-documents-api` | `aa52ec1` | #36 기준 draft stacked PR 생성 완료. 로컬 검증 통과. GitHub Actions checks 없음. base #36에는 #28의 stacked PR CI workflow가 아직 포함되지 않음 |
| #40 | `[feat] 자료 분석 작업 생성 API 추가` | `feature/analyze-resource-job-api` | `9904dda` | #37 기준 draft stacked PR 생성 완료. 로컬 검증 통과. GitHub Actions checks 없음. base #37에는 #28의 stacked PR CI workflow가 아직 포함되지 않음 |
...
| 에이전트 작업 생성 | `POST /api/ai/analyze-resource`는 자료 요약, 임베딩, AI 문서 분류 작업 생성 | `feature/analyze-resource-job-api`에서 자료 접근 확인 후 `agent_jobs` PENDING job 생성까지만 처리한다 |
...
| AI 문서 목록 | `GET /api/project-rooms/{roomId}/ai-documents`는 프로젝트룸 AI 문서 분석 목록을 반환 | `feature/room-ai-documents-api`에서 프로젝트룸 ACTIVE 멤버 권한 확인 후 `ai_documents` 목록을 반환하도록 보정한다 |
...
8. #37은 #36 merge 후 `develop` 기준으로 GitHub Actions CI를 재확인한다.
9. #40은 #37 merge 후 `develop` 기준으로 GitHub Actions CI를 재확인한다.
10. 추천 다음 작업은 Entity/Flyway 정합성 점검이다. 특히 `agent_model_call_logs`, `agent_suggestions`, `resource` 관련 테이블을 먼저 본다.
10. #19~#26은 현재 API 기준으로 계속 진행하되, 수정본 API가 오면 차이 보정한다.
