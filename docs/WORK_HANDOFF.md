...
Last checked: 2026-06-24 21:25:32 KST
...
| 현재 확인 브랜치 | `feature/auth-google-foundation` |
| 원격 기준 브랜치 | `develop` |
...
| #24 | `chore: Google-only 인증 기반 정리` | `feature/auth-google-foundation` | `cd88273` | #23 기준 draft stacked PR 생성됨. GitHub Actions checks 없음. #22, #23 merge 후 `develop` 기준 CI 재확인 필요 |
...
3. `docs/CURRENT_API_BASELINE_WORK.md` 기준으로 다음 작업 1개를 선택한다.
4. 추천 다음 작업은 `work.task` 기본 API 구현이다.
5. #19~#24는 현재 API 기준으로 계속 진행하되, 수정본 API가 오면 차이 보정한다.
6. 열린 PR 정리가 끝난 뒤 자료/AI 문서/RAG API로 넘어간다.
