...
Last checked: 2026-06-25 17:43 KST
...
| 현재 확인 브랜치 | `feature/storage-usage-api` |
| 원격 기준 브랜치 | `develop` |
...
| #31 | `[feat] 자료 관련 문서 조회 API 추가` | `feature/resource-related-api` | latest pushed | #25 기준 draft stacked PR 생성 완료. 로컬 검증 통과. GitHub Actions checks 없음. base #25에는 #28의 stacked PR CI workflow가 아직 포함되지 않음 |
| #39 | `[feat] 저장 용량 조회 API 추가` | `feature/storage-usage-api` | `944db4d` | #31 기준 draft stacked PR 생성 완료. 로컬 검증 통과. GitHub Actions checks 없음. base #31에는 #28의 stacked PR CI workflow가 아직 포함되지 않음 |
...
4. #39는 #31 merge 후 `develop` 기준으로 GitHub Actions CI를 재확인한다.
5. 다음 작업은 파일 업로드/download-url의 Storage Service 선행 조건을 별도 PR로 나눌지 검토한다.
6. #19~#25, #31은 현재 API 기준으로 계속 진행하되, 수정본 API가 오면 차이 보정한다.
7. #26은 #22, #23, #24, #25 merge 후 `develop` 기준으로 GitHub Actions CI를 재확인한다.
