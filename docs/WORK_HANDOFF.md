...
Last checked: 2026-06-24 21:07:12 KST
...
| 현재 확인 브랜치 | `feature/schedule-basic-api` |
| 원격 기준 브랜치 | `develop` |
...
- `./gradlew compileTestJava` 통과 (2026-06-24 20:20:42 KST)
- `./gradlew cleanTest test` 통과 (2026-06-24 20:20:42 KST)
- `git diff --check` 통과 (2026-06-24 20:20:42 KST)
...
| 생성 필요 | `feat: 프로젝트룸 권한 검사 서비스 분리` | `feature/room-access-service` | `36a3f5d` | #22 기준 stacked PR로 생성 필요. 원격 브랜치는 push됨. GitHub 커넥터 PR 생성이 403 권한 오류로 실패함 |
...
1. `feature/room-access-service` 원격 브랜치에서 #22 `feature/schedule-basic-api` 대상 draft PR을 생성한다.
2. PR 본문에는 로컬 검증 통과와 stacked PR이라 CI가 자동 실행되지 않을 수 있다는 점을 남긴다.
3. #22 merge 후 `develop` 기준으로 GitHub Actions CI를 재확인한다.
4. 이후 `docs/CURRENT_API_BASELINE_WORK.md` 기준으로 다음 작업 1개를 선택한다.
5. 추천 다음 작업은 인증 기반 정리 또는 `work.task` 기본 API 구현이다.
4. #19~#22는 현재 API 기준으로 계속 진행하되, 수정본 API가 오면 차이 보정한다.
5. 열린 PR 정리가 끝난 뒤 자료/AI 문서/RAG API로 넘어간다.
