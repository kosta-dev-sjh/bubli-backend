...
Last checked: 2026-06-25 17:45 KST
...
- #39 이후 storage usage를 사용하는 resource upload stack은 #64/#67 순서로 다시 확인한다.
...
- #67은 #64 head `637fa26` 기준으로 mergeState가 `UNKNOWN`이라 다음 resource delete stack 확인 대상이다.
...
| #67 | `[feat] 자료 삭제 저장소 객체 정리 추가` | `feature/resource-delete-storage-cleanup` | `feature/resource-upload-storage-usage` | `0fac143` | checks 없음, merge unknown, ready | #64 head `637fa26` 갱신 후 다음 확인 대상. 자료 삭제 시 연결 storage object best-effort 삭제 추가 |
...
6. #67은 #64 최신 head `637fa26` 기준으로 병합/충돌 정리 후 로컬 검증을 확인한다.
7. #39는 최신 #31 head `3bf14f5` 기준 충돌 해소와 ready 전환이 끝났다. 다음 resource delete stack은 #67부터 확인한다.
8. #67 이후 resource delete stack은 #67 최신 head 기준으로 순차 재검증한다.
