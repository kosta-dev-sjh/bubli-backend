...
- PR별 상태는 `docs/API_SKELETON_PR_MATRIX.csv`에서 `type`, `status`, `review_priority`, `team_action`으로 확인한다.
...
- 팀원이 `develop` 병합을 진행 중이면, `develop`에 바로 병합 가능한 작은 변경을 먼저 찾는다.
- 이미 merge된 stack base 위의 PR은 기존 PR을 억지로 retarget하지 않고, 필요한 경우 최신 `origin/develop` 위 새 `codex/*` 브랜치로 순수 변경분만 재구성해서 develop 대상 PR로 올린다.
...
- #31 resource related API 최신 #25 base 재병합, `WORK_HANDOFF.md` 충돌 해결, 로컬 검증 통과. GitHub checks 없음 (base #25에 stacked PR CI workflow 없음)
- #39 storage usage API 최신 #31 base 병합, `RoomMemberRepository.java`/`ErrorCode.java`/`WORK_HANDOFF.md` 충돌 해결, ArchUnit 공개 서비스 경계 보정 후 로컬 검증 통과
...
- #50 user privacy consents API 최신 #49 base 병합, ProjectMembershipPublicService 공개 경계 유지, ArchitectureTest 로컬 검증 통과. GitHub checks 없음 (base #49에 stacked PR CI workflow 없음)
- #51 user project rooms API 로컬 검증 통과. GitHub checks 없음 (base #50에 stacked PR CI workflow 없음)
- #52 S3 download-url provider 최신 #46 base 병합, ArchitectureTest 로컬 검증 통과. GitHub checks 없음 (base #46에 stacked PR CI workflow 없음)
...
- #54 S3 storage service boundary 최신 #52 base 병합, ArchitectureTest 로컬 검증 통과. GitHub checks 없음 (base #52에 stacked PR CI workflow 없음)
- #55 resource multipart upload API 최신 #54 base 병합, StoragePublicService 경계 보정, ArchitectureTest 로컬 검증 통과. GitHub checks 없음 (base #54에 stacked PR CI workflow 없음)
- #56 resource upload compensation 최신 #55 base 병합, StoragePublicService 보상 삭제 경계 보정, ArchitectureTest 로컬 검증 통과. GitHub checks 없음 (base #55에 stacked PR CI workflow 없음)
- #57 resource upload policy 최신 #56 base 병합, StoragePublicService/보상 삭제 흐름 유지, ArchitectureTest 로컬 검증 통과. GitHub checks 없음 (base #56에 stacked PR CI workflow 없음)
- #64 resource upload/storage usage 연결 로컬 검증 통과. GitHub checks 없음 (base #57에 stacked PR CI workflow 없음)
- #64 resource upload/storage usage 최신 #57 base 병합, StorageUsagePublicService 공개 경계 보정, ArchitectureTest 로컬 검증 통과. GitHub checks 없음 (base #57에 stacked PR CI workflow 없음)
- #83 time-log 기본 API를 최신 `develop` 기준 PR로 재구성. 로컬 검증과 GitHub Actions `build` 통과
- #85 project room management API를 최신 `develop` 기준 PR로 재구성. 로컬 검증과 GitHub Actions `build` 통과
- #47 user profile update API에 최신 `develop` 병합, ArchUnit 기준 반영, ready 전환. 로컬 검증과 GitHub Actions `build` 통과
- #48 user preferences API에 최신 #47 base 병합, ProjectMembershipPublicService 공개 경계 보정. 로컬 검증 통과. GitHub checks 없음 (base #47인 stacked PR)
- #49 user notification preferences API에 최신 #48 base 병합. 로컬 검증 통과. GitHub checks 없음 (base #48인 stacked PR)
- #67 resource delete storage cleanup 최신 #64 base 병합, ResourceServiceTest 충돌 해결, 로컬 검증 통과. GitHub checks 없음 (base #64에 stacked PR CI workflow 없음)
...
### 작업 카드 AB-4. WbsItemPublicService 공개 계약 분리

처리 시각: 2026-06-25 19:06 KST

브랜치: `codex/abstraction-boundary-wbs-public-service`

변경 내용:

- #29 `chore/latest-docs-2026-06-25` 기준으로 `WbsItemPublicService`를 인터페이스 계약으로 분리했다.
- 기존 WBS 항목 room 소속 검증 구현은 `WbsItemPublicServiceImpl`로 옮겼다.
- `work.task` 도메인은 기존 이름인 `WbsItemPublicService` 인터페이스를 계속 주입받는다.
...
- `git diff --check` 통과
- PR #89 head `317c27e` 기준 GitHub checks 없음. 현재 CI workflow의 `pull_request.branches`가 `develop`, `main`, `feature/**`만 포함하고, PR #89 base가 `chore/latest-docs-2026-06-25`라서 check run이 생성되지 않는다.

후속 PR stack 반영 순서:

1. #29 문서 PR 뒤에 이 보정 PR을 반영한다.
2. `WbsItemPublicService`를 참조하는 task/WBS stack은 이 PR 반영 뒤 최신 base를 병합하거나 필요한 최소 충돌만 해결한다.
3. 후속 PR에서 구현체를 직접 주입하는 코드가 생기면 `WbsItemPublicService` 인터페이스 주입으로 되돌린다.
...
처리 시각: 2026-06-25 17:49 KST

변경 내용:

- #67 `feature/resource-delete-storage-cleanup` 브랜치에 최신 #64 `feature/resource-upload-storage-usage` head `637fa26`를 병합했다.
- `ResourceServiceTest.java` 충돌을 해결했다.
- 삭제 시 저장소 객체 best-effort 삭제, soft delete 상태 유지, 저장공간 사용량 해제 검증을 모두 보존했다.
- `ResourceService`의 저장소 삭제 호출을 최신 `StoragePublicService` 공개 경계 이름으로 맞췄다.
- #67 PR 본문을 최신 head, 로컬 검증 결과, stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #67: `./gradlew test --tests '*ArchitectureTest'` 통과
- #67: `./gradlew compileTestJava` 통과
- #67: `./gradlew cleanTest test` 통과
- #67: `git diff --check` 통과
- #67: head `8c553ca`, base `feature/resource-upload-storage-usage`, draft 아님, mergeState `CLEAN`
- #67: GitHub checks 없음. base가 `feature/resource-upload-storage-usage`인 stacked PR이라 check run이 보고되지 않음

메모:

- #67은 다시 `CLEAN`/ready 상태로 복구됐다.
- resource delete stack은 #75부터 최신 #67 기준으로 순차 재확인한다.

### 작업 카드 39-3. #39 저장 용량 API 최신 #31 기준 정리

처리 시각: 2026-06-25 17:45 KST

변경 내용:

- #39 `feature/storage-usage-api` 브랜치에 최신 #31 `feature/resource-related-api` head `3bf14f5`를 병합했다.
- `RoomMemberRepository.java`, `ErrorCode.java`, `docs/WORK_HANDOFF.md` 충돌을 해결했다.
- `StorageUsageService`의 project Repository/Entity 직접 의존을 제거하고 `ProjectMembershipPublicService.findActiveRoomIds(...)` 공개 경계로 보정했다.
- #39 PR 본문을 최신 head, 로컬 검증 결과, stacked PR checks 없음 사유로 갱신했다.
- #39를 draft에서 ready로 전환했다.

검증 결과:

- #39: `./gradlew test --tests '*ArchitectureTest'` 1차 실패 후 공개 서비스 경계 보정으로 재실행 통과
- #39: `./gradlew compileTestJava` 통과
- #39: `./gradlew cleanTest test` 통과
- #39: `git diff --check` 통과
- #39: head `05721e4`, base `feature/resource-related-api`, draft 아님, mergeState `CLEAN`
- #39: GitHub checks 없음. base가 `feature/resource-related-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #39는 다시 `CLEAN`/ready 상태로 복구됐다.
- #39 이후 storage usage를 사용하는 resource upload/delete stack은 #64, #67까지 다시 확인했다.

### 작업 카드 31-3. #31 자료 관련 문서 조회 API 최신 #25 기준 재정리

처리 시각: 2026-06-25 17:39 KST

변경 내용:

- #31 `feature/resource-related-api` 브랜치에 최신 #25 `feature/resource-basic-foundation` head `b609674`를 병합했다.
- 충돌은 `docs/WORK_HANDOFF.md` 기록 영역에서만 발생했고, #31 resource related 기록과 선행 agent 기록을 모두 보존해 해결했다.
- #31 PR 본문을 최신 head, 로컬 검증 결과, stacked PR checks 없음 사유로 갱신했다.
...
- #31: head `3bf14f5`, base `feature/resource-basic-foundation`, draft 아님, mergeState `CLEAN`
- #31: GitHub checks 없음. base가 `feature/resource-basic-foundation`인 stacked PR이라 check run이 보고되지 않음

메모:

- #31은 다시 `CLEAN` 상태로 복구됐다.
- #39는 최신 #31 head `3bf14f5` 기준으로 다음 충돌 정리 대상이다.

### 작업 카드 51-2. #51 내 프로젝트룸 목록 API 최신 #50 기준 정리

처리 시각: 2026-06-25 17:32 KST

변경 내용:

- #51 `feature/user-project-rooms-api` 브랜치에 최신 #50 `feature/user-privacy-consents-api` head를 병합한 상태를 재확인했다.
- `ProjectRoomControllerIntegrationTest.java` 충돌 해결 커밋이 원격에 push된 상태를 확인했다.
- #51 PR 본문에는 현재 API 기준 구현, 최종 API 수정본 수신 시 보정 가능 부분, 로컬 검증, stacked PR checks 없음 사유가 기록돼 있다.
- 이번 문서 브랜치에서는 #51 head와 상태표만 실제 GitHub 상태에 맞게 갱신한다.

검증 결과:

- #51: PR 본문 기준 `./gradlew test --tests '*ArchitectureTest'` 통과
- #51: PR 본문 기준 `./gradlew compileTestJava` 통과
- #51: PR 본문 기준 `./gradlew cleanTest test` 통과
- #51: PR 본문 기준 `git diff --check` 통과
- #51: head `f16f888`, base `feature/user-privacy-consents-api`, draft 아님, mergeState `CLEAN`
- #51: GitHub checks 없음. base가 `feature/user-privacy-consents-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #51은 #50 뒤에 이어서 볼 user/settings stack 후보로 정리됐다.
- user/settings stack #47~#51은 최신 상태표 기준으로 모두 ready 또는 clean 상태다.

### 작업 카드 50-2. #50 사용자 개인정보 동의 API 최신 #49 기준 정리

처리 시각: 2026-06-25 16:40 KST

변경 내용:

- #50 `feature/user-privacy-consents-api` 브랜치에 최신 #49 `feature/user-notification-preferences-api` head `6f53d4e`를 병합했다.
- `UserService.java`, `UserServiceTest.java` 충돌을 해결했다.
- #50의 개인정보 동의 조회/수정 API와 #49의 알림 설정 조회/수정 API를 모두 보존했다.
- 프로젝트룸 권한 확인은 다른 도메인 Service 직접 의존 없이 `ProjectMembershipPublicService` 공개 경계만 사용하도록 유지했다.
- #50 PR 본문에 현재 API 기준 구현, 기본 API 골격 범위, 최종 API 수정본 수신 시 보정 가능 부분, 로컬 검증, stacked PR checks 없음 사유를 기록했다.

검증 결과:

- #50: `./gradlew test --tests '*ArchitectureTest'` 통과
- #50: `./gradlew compileTestJava` 통과
- #50: `./gradlew cleanTest test` 통과
- #50: `git diff --check` 통과
- #50: head `dbfc595`, base `feature/user-notification-preferences-api`, draft 아님, mergeState `CLEAN`
- #50: GitHub checks 없음. base가 `feature/user-notification-preferences-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #50은 완성 기능이 아니라 팀원이 이어서 구현할 수 있는 컴파일/테스트 가능한 기본 API 골격이다.
- #51은 최신 #50 기준으로 병합/검증 상태가 확인됐다. 현재 #51 head는 `f16f888`이다.

### 작업 카드 49-2. #49 사용자 알림 설정 API 최신 #48 기준 정리

처리 시각: 2026-06-25 16:33 KST

변경 내용:

- #49 `feature/user-notification-preferences-api` 브랜치에 최신 #48 `feature/user-preferences-api` head `82a515e`를 병합했다.
- `UserService.java`, `UserServiceTest.java` 충돌을 해결했다.
- #49의 알림 설정 조회/수정 API와 #48의 `ProjectMembershipPublicService` 공개 경계 보정을 모두 보존했다.
- #49 PR 본문에 현재 API 기준 구현, 최종 API 수정본 수신 시 보정 가능 부분, 로컬 검증, stacked PR checks 없음 사유를 기록했다.

검증 결과:

- #49: `./gradlew test --tests '*ArchitectureTest'` 통과
- #49: `./gradlew compileTestJava` 통과
- #49: `./gradlew cleanTest test` 통과
- #49: `git diff --check` 통과
- #49: head `6f53d4e`, base `feature/user-preferences-api`, draft 아님, mergeState `CLEAN`
- #49: GitHub checks 없음. base가 `feature/user-preferences-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #49는 #48이 앞선 user/settings stack으로 정리된 뒤 다음 user/settings stack 후보로 재확인한다.
- #50은 최신 #49 head `6f53d4e` 기준으로 다음 user/settings stack 확인 대상이다.

### 작업 카드 48-2. #48 사용자 설정 API 최신 #47 기준 정리

처리 시각: 2026-06-25 16:24 KST

변경 내용:

- #48 `feature/user-preferences-api` 브랜치에 최신 #47 `feature/user-me-update-api` head `bef21d7`를 병합했다.
- `UserService`의 project 일반 Service 직접 의존을 제거하고 `ProjectMembershipPublicService.assertActiveMember` 공개 경계로 보정했다.
- `GET/PATCH /api/me/preferences` 기본 API 뼈대와 `default_project_room_id` V2 migration은 유지했다.
- #48 PR 본문에 현재 API 기준 구현, 최종 API 수정본 수신 시 보정 가능 부분, 로컬 검증, stacked PR checks 없음 사유를 기록했다.

검증 결과:

- #48: `./gradlew test --tests '*ArchitectureTest'` 1차 실패 후 공개 서비스 경계 보정으로 재실행 통과
- #48: `./gradlew compileTestJava` 통과
- #48: `./gradlew cleanTest test` 통과
- #48: `git diff --check` 통과
- #48: head `82a515e`, base `feature/user-me-update-api`, draft 아님, mergeState `CLEAN`
- #48: GitHub checks 없음. base가 `feature/user-me-update-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #48은 #47이 `develop`에 merge된 뒤 다음 user/settings stack 후보로 재확인한다.
- #49는 최신 #48 head `82a515e` 기준으로 다음 user/settings stack 확인 대상이다.

### 작업 카드 47-2. #47 내 프로필 수정 API 최신 develop 기준 정리

처리 시각: 2026-06-25 16:13 KST

변경 내용:

- #47 `feature/user-me-update-api` 브랜치에 최신 `origin/develop`을 병합했다.
- 이전 draft 유지 사유였던 최신 develop/ArchUnit 기준 미반영을 해소했다.
- `PATCH /api/me` 기본 API 뼈대는 유지하고, 사용자 프로필 필드 수정 범위만 다룬다.
- #47 PR 본문에 현재 API 기준 구현, 최종 API 수정본 수신 시 보정 가능 부분, 로컬 검증, GitHub Actions CI 결과를 기록했다.
- #47을 draft에서 ready로 전환했다.

검증 결과:

- #47: `./gradlew test --tests '*ArchitectureTest'` 통과
- #47: `./gradlew compileTestJava` 통과
- #47: `./gradlew cleanTest test` 통과
- #47: `git diff --check` 통과
- #47: GitHub Actions CI `build` 통과, Run ID `28153322818`, Job ID `83375854203`
- #47: head `bef21d7`, base `develop`, draft 아님, review required, mergeState `BLOCKED`

메모:

- #47은 `develop` 대상이고 CI가 통과했으므로 팀 리뷰 후 병합 검토 가능한 PR이다.
- `mergeState BLOCKED`는 현재 리뷰 required 상태로 보이며, 충돌/CI 실패 상태는 아니다.

### 작업 카드 85. #58 프로젝트룸 수정 결제 종료 API develop 대상 재구성

처리 시각: 2026-06-25 16:08 KST

변경 내용:

- 기존 #58 `feature/project-room-management-api`의 순수 변경분을 최신 `origin/develop` 위 `codex/develop-project-room-management-api` 브랜치로 재구성했다.
- #58 base였던 #19 프로젝트룸 멤버/초대 API는 이미 `develop`에 merge된 상태라, develop 병합용으로 새 PR #85를 생성했다.
- 프로젝트룸 수정, 계약/입금 정보 수정, 프로젝트룸 종료 기본 API 뼈대만 반영했다.
- `description`은 현재 서버 DB 컬럼 기준에 없어 이번 뼈대에는 넣지 않았다.
- #85 PR 본문에 현재 API 기준 구현, 최종 API 수정본 수신 시 보정 가능 부분, 로컬 검증, GitHub Actions CI 결과를 기록했다.

검증 결과:

- #85: `./gradlew test --tests '*ArchitectureTest'` 통과
- #85: `./gradlew compileTestJava` 통과
- #85: `./gradlew cleanTest test` 통과
- #85: `git diff --check` 통과
- #85: GitHub Actions CI `build` 통과, Run ID `28153058305`, Job ID `83374996118`
- #85: head `c404b02`, base `develop`, draft 아님, review required, mergeState `BLOCKED`

메모:

- #85는 `develop` 대상이고 CI가 통과했으므로 팀 리뷰 후 병합 검토 가능한 PR이다.
- `mergeState BLOCKED`는 현재 리뷰 required 상태로 보이며, 충돌/CI 실패 상태는 아니다.

### 작업 카드 83. #30 타이머 작업시간 기본 API develop 대상 재구성

처리 시각: 2026-06-25 16:01 KST

변경 내용:

- 기존 #30 `feature/time-log-basic-api`의 순수 변경분을 최신 `origin/develop` 위 `codex/develop-time-log-basic-api` 브랜치로 재구성했다.
- #30 cherry-pick 중 `ErrorCode.java` 충돌이 있었고, develop의 CHAT 에러 코드를 유지하면서 PERSONAL 타이머 에러 코드를 추가했다.
- `TimeLogService`의 project `RoomMemberRepository` 직접 의존을 제거하고 `ProjectMembershipPublicService.assertActiveMember` 공개 경계로 보정했다.
- 기존 #30 stacked PR은 그대로 두고, develop에 바로 리뷰/병합 가능한 새 PR #83을 생성했다.
- #83 PR 본문에 현재 API 기준 구현, 최종 API 수정본 수신 시 보정 가능 부분, 로컬 검증, GitHub Actions CI 결과를 기록했다.

검증 결과:

- #83: `./gradlew test --tests '*ArchitectureTest'` 통과
- #83: `./gradlew compileTestJava` 통과
- #83: `./gradlew cleanTest test` 통과
- #83: `git diff --check` 통과
- #83: GitHub Actions CI `build` 통과, Run ID `28152720768`, Job ID `83373932116`
- #83: head `774502c`, base `develop`, draft 아님, review required, mergeState `BLOCKED`

메모:

- #83은 `develop` 대상이고 CI가 통과했으므로 팀 리뷰 후 병합 검토 가능한 PR이다.
- `mergeState BLOCKED`는 현재 리뷰 required 상태로 보이며, 충돌/CI 실패 상태는 아니다.
- 앞으로 develop 병합 가능성이 높은 작업은 기존 stacked PR을 그대로 누르기보다 최신 `develop` 위 재구성 PR을 먼저 만든다.

### 작업 카드 64-2. #64 자료 업로드 저장공간 사용량 연결 최신 #57 기준 정리

처리 시각: 2026-06-25 15:49 KST

변경 내용:

- #64 `feature/resource-upload-storage-usage` 브랜치에 최신 #57 `feature/resource-upload-policy` head `60b9470`를 병합했다.
- `RoomMemberRepository.java`, `ResourceService.java`, `ResourceServiceTest.java` 충돌을 해결했다.
- 최신 #57의 `StoragePublicService`, `ProjectMembershipPublicService`, ResourceStatus 삭제 정책을 유지했다.
- #64의 저장공간 사용량 기록/보상 해제/삭제 해제 흐름을 보존했다.
- resource 도메인이 storage 내부 service를 직접 의존하지 않도록 `StorageUsagePublicService` 공개 경계를 추가했다.
- storage usage 조회에서 project repository/entity 직접 참조를 제거하고 `ProjectMembershipPublicService` 공개 경계를 사용하도록 정리했다.
- #64 PR 본문을 최신 기준 문서, 로컬 검증, stacked PR checks 없음 사유로 갱신하고 ready로 전환했다.

검증 결과:

- #64: `./gradlew test --tests '*ArchitectureTest'` 통과
- #64: `./gradlew compileTestJava` 통과
- #64: `./gradlew cleanTest test` 통과
- #64: `git diff --check` 통과
- #64: head `637fa26`, base `feature/resource-upload-policy`, mergeState `CLEAN`, ready
- #64: GitHub checks 없음. base가 `feature/resource-upload-policy`인 stacked PR이라 check run이 보고되지 않음

메모:

- 남은 draft PR은 당시 기준 #39, #47, #62였다. 현재 #39와 #47은 ready로 전환됐고, 남은 draft PR은 #62다.
- #67은 2026-06-25 17:49 KST에 최신 #64 기준으로 다시 `CLEAN` 상태가 됐다.
...
- 이후 #47, #64, #39는 ready로 전환됐다. 현재 남은 draft PR은 #62다.
...
- 현재 상태는 열린 PR 상태 표와 최신 작업 카드를 기준으로 본다. #39, #47, #64는 ready 전환 완료, #62는 draft 유지.

### 작업 카드 57-2. #57 자료 업로드 정책 검사 최신 #56 기준 정리

처리 시각: 2026-06-25 15:26 KST

변경 내용:

- #57 `feature/resource-upload-policy` 브랜치에 최신 #56 `feature/resource-upload-compensation` head `6d29e0d`를 병합했다.
- 병합 충돌은 없었다.
- #57의 업로드 크기/MIME 정책 검사와 #56의 저장소 보상 삭제 흐름을 모두 보존했다.
- storage 도메인 의존은 `StoragePublicService` 공개 경계로 유지했다.
- #57 자체 변경으로 `V1__initial_schema.sql`이나 `PostgresIntegrationTestSupport.java`를 건드리지 않는 상태로 확인했다.
- #57 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #57: `./gradlew test --tests '*ArchitectureTest'` 통과
- #57: `./gradlew compileTestJava` 통과
- #57: `./gradlew cleanTest test` 통과
- #57: `git diff --check` 통과
- #57: head `60b9470`, base `feature/resource-upload-compensation`, mergeState `CLEAN`
- #57: GitHub checks 없음. base가 `feature/resource-upload-compensation`인 stacked PR이라 check run이 보고되지 않음

메모:

- #57은 draft PR 상태를 유지한다.
- #64는 #57 head `60b9470` 기준으로 다음 resource upload/storage usage stack 확인 대상이다.

### 작업 카드 56-2. #56 자료 업로드 실패 보상 삭제 최신 #55 기준 정리

처리 시각: 2026-06-25 15:21 KST

변경 내용:

- #56 `feature/resource-upload-compensation` 브랜치에 최신 #55 `feature/resource-multipart-upload-api` head `cb94fa1`를 병합했다.
- `ResourceService.java` 충돌을 해결했다.
- #56의 저장소 보상 삭제 try/catch 흐름과 #55의 `StoragePublicService` 공개 경계를 모두 보존했다.
- 업로드 파일 저장 성공 후 DB file/version 메타데이터 저장 실패 시 `StoragePublicService.delete`로 저장소 객체를 정리한다.
- 보상 삭제 실패는 원래 예외에 suppressed exception으로 붙이고 원래 예외를 전파하는 정책을 유지했다.
- #56 자체 변경으로 `V1__initial_schema.sql`이나 `PostgresIntegrationTestSupport.java`를 건드리지 않는 상태로 확인했다.
- #56 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #56: `./gradlew test --tests '*ArchitectureTest'` 통과
- #56: `./gradlew compileTestJava` 통과
- #56: `./gradlew cleanTest test` 통과
- #56: `git diff --check` 통과
- #56: head `6d29e0d`, base `feature/resource-multipart-upload-api`, mergeState `CLEAN`
- #56: GitHub checks 없음. base가 `feature/resource-multipart-upload-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #56은 draft PR 상태를 유지한다.
- #57은 #56 head `6d29e0d` 기준으로 다음 resource upload policy stack 확인 대상이다.

### 작업 카드 55-2. #55 자료 multipart 업로드 API 최신 #54 기준 정리

처리 시각: 2026-06-25 15:13 KST

변경 내용:

- #55 `feature/resource-multipart-upload-api` 브랜치에 최신 #54 `feature/s3-storage-service-boundary` head `0f48244`를 병합했다.
- `ResourceService.java`, `ResourceServiceTest.java` 충돌을 해결했다.
- #55의 multipart upload 흐름과 #54의 S3 저장/삭제 경계를 모두 보존했다.
- 오래된 `RoomAccessService` 참조는 제거하고 `ProjectMembershipPublicService` 권한 경계를 유지했다.
- resource 도메인이 storage 도메인의 일반 service를 직접 의존하지 않도록 `StoragePublicService` 공개 포트를 추가했다.
- `S3StorageService`, `DisabledStorageService`는 `StoragePublicService` 구현체로 보정했다.
- #55 자체 변경으로 `V1__initial_schema.sql`이나 `PostgresIntegrationTestSupport.java`를 건드리지 않는 상태로 확인했다.
- #55 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #55: `./gradlew test --tests '*ArchitectureTest'` 1차 실패 후 `StoragePublicService` 경계 보정으로 재실행 통과
- #55: `./gradlew compileTestJava` 통과
- #55: `./gradlew cleanTest test` 통과
- #55: `git diff --check` 통과
- #55: head `cb94fa1`, base `feature/s3-storage-service-boundary`, mergeState `CLEAN`
- #55: GitHub checks 없음. base가 `feature/s3-storage-service-boundary`인 stacked PR이라 check run이 보고되지 않음

메모:

- #55는 draft PR 상태를 유지한다.
- #56은 #55 head `cb94fa1` 기준으로 다음 resource upload compensation stack 확인 대상이다.

### 작업 카드 54-2. #54 S3 저장 서비스 경계 최신 #52 기준 정리

처리 시각: 2026-06-25 15:04 KST

변경 내용:

- #54 `feature/s3-storage-service-boundary` 브랜치에 최신 #52 `feature/s3-download-url-provider` head `31d9eb8`를 병합했다.
- 병합 충돌은 없었다.
- `StorageService`, `S3StorageService`, disabled fallback 경계가 유지되는 상태로 확인했다.
- S3 객체를 public으로 열지 않고, 후속 upload/download API가 서버 권한 확인 뒤 storage 경계를 호출하는 구조를 유지했다.
- #54 자체 변경으로 `V1__initial_schema.sql`이나 `PostgresIntegrationTestSupport.java`를 건드리지 않는 상태로 확인했다.
- #54 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #54: `./gradlew test --tests '*ArchitectureTest'` 통과
- #54: `./gradlew compileTestJava` 통과
- #54: `./gradlew cleanTest test` 통과
- #54: `git diff --check` 통과
- #54: head `0f48244`, base `feature/s3-download-url-provider`, mergeState `CLEAN`
- #54: GitHub checks 없음. base가 `feature/s3-download-url-provider`인 stacked PR이라 check run이 보고되지 않음

메모:

- #54는 draft PR 상태를 유지한다.
- #55는 #54 head `0f48244` 기준으로 다음 resource multipart upload stack 확인 대상이다.

### 작업 카드 52-2. #52 S3 다운로드 URL Provider 최신 #46 기준 정리

처리 시각: 2026-06-25 14:58 KST

변경 내용:

- #52 `feature/s3-download-url-provider` 브랜치에 최신 #46 `feature/resource-download-url-api` head `c5632d8`를 병합했다.
- 병합 충돌은 없었다.
- S3 presigned download URL Provider와 disabled fallback Provider 경계가 유지되는 상태로 확인했다.
- 다운로드 권한 확인은 #46의 resource service 흐름에서 먼저 수행하고, #52는 저장소 URL 발급 Provider만 담당하는 상태로 확인했다.
- #52 자체 변경으로 `V1__initial_schema.sql`이나 `PostgresIntegrationTestSupport.java`를 건드리지 않는 상태로 확인했다.
- #52 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #52: `./gradlew test --tests '*ArchitectureTest'` 통과
- #52: `./gradlew compileTestJava` 통과
- #52: `./gradlew cleanTest test` 통과
- #52: `git diff --check` 통과
- #52: head `31d9eb8`, base `feature/resource-download-url-api`, mergeState `CLEAN`
- #52: GitHub checks 없음. base가 `feature/resource-download-url-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #52는 draft PR 상태를 유지한다.
- #54는 #52 head `31d9eb8` 기준으로 다음 S3 storage service boundary stack 확인 대상이다.
...
처리 시각: 2026-06-25 14:41 KST

변경 내용:

- #44 `feature/generate-questions-job-api` 브랜치에 최신 #43 `feature/generate-wbs-job-api` head `1145670`를 병합했다.
- 병합 충돌은 없었다.
- generate-questions 흐름의 오래된 `RoomAccessService` 호출 흔적을 `ProjectMembershipPublicService.assertActiveMember`로 보정했다.
- agent 패키지는 다른 도메인의 Repository/Entity/일반 Service를 직접 참조하지 않는 상태로 확인했다.
- 확인 질문 후보 생성 작업은 `agent_jobs` PENDING 생성까지만 처리하며, 확정 업무 데이터 저장은 하지 않는 상태로 확인했다.
- `PostgresIntegrationTestSupport.java`와 `V1__initial_schema.sql`은 develop 대비 diff가 없는 상태로 확인했다.
- #44 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #44: `./gradlew test --tests '*ArchitectureTest'` 통과
- #44: `./gradlew compileTestJava` 통과
- #44: `./gradlew cleanTest test` 통과
- #44: `git diff --check` 통과
- #44: head `2ac6438`, base `feature/generate-wbs-job-api`, mergeState `CLEAN`
- #44: GitHub checks 없음. base가 `feature/generate-wbs-job-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #44는 draft PR 상태를 유지한다.
- #45는 #44 head `2ac6438` 기준으로 다음 agent job 생성 stack 확인 대상이다.

### 작업 카드 43-2. #43 WBS 후보 생성 작업 API 최신 #42 기준 정리

처리 시각: 2026-06-25 14:36 KST

변경 내용:

- #43 `feature/generate-wbs-job-api` 브랜치에 최신 #42 `feature/generate-tasks-job-api` head `8bdc107`를 병합했다.
- 병합 충돌은 없었다.
- generate-wbs 흐름의 오래된 `RoomAccessService` 호출 흔적을 `ProjectMembershipPublicService.assertActiveMember`로 보정했다.
- agent 패키지는 다른 도메인의 Repository/Entity/일반 Service를 직접 참조하지 않는 상태로 확인했다.
- WBS 후보 생성 작업은 `agent_jobs` PENDING 생성까지만 처리하며, `wbs_items` 확정 저장은 하지 않는 상태로 확인했다.
- `PostgresIntegrationTestSupport.java`와 `V1__initial_schema.sql`은 develop 대비 diff가 없는 상태로 확인했다.
- #43 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #43: `./gradlew test --tests '*ArchitectureTest'` 통과
- #43: `./gradlew compileTestJava` 통과
- #43: `./gradlew cleanTest test` 통과
- #43: `git diff --check` 통과
- #43: head `1145670`, base `feature/generate-tasks-job-api`, mergeState `CLEAN`
- #43: GitHub checks 없음. base가 `feature/generate-tasks-job-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #43은 draft PR 상태를 유지한다.
- #44는 #43 head `1145670` 기준으로 다음 agent job 생성 stack 확인 대상이다.
...
처리 시각: 2026-06-25 14:22 KST

변경 내용:

- #41 `feature/generate-requirements-job-api` 브랜치에 최신 #40 `feature/analyze-resource-job-api` head `e2b7e5c`를 병합했다.
- `src/main/java/com/bubli/agent/service/AiJobCommandService.java`에서 충돌이 발생했지만, analyze-resource와 generate-requirements 흐름을 모두 보존하는 방향으로 수동 병합했다.
- analyze-resource 흐름은 `ResourcePublicService.getReadableResource` 경계를 유지했다.
- generate-requirements 흐름의 오래된 `RoomAccessService` 의존을 `ProjectMembershipPublicService.assertActiveMember`로 보정했다.
- agent 패키지는 다른 도메인의 Repository/Entity/일반 Service를 직접 참조하지 않는 상태로 확인했다.
- `PostgresIntegrationTestSupport.java`와 `V1__initial_schema.sql`은 develop 대비 diff가 없는 상태로 확인했다.
- #41 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #41: `./gradlew test --tests '*ArchitectureTest'` 통과
- #41: `./gradlew compileTestJava` 통과
- #41: `./gradlew cleanTest test` 통과
- #41: `git diff --check` 통과
- #41: head `4088d62`, base `feature/analyze-resource-job-api`, mergeState `CLEAN`
- #41: GitHub checks 없음. base가 `feature/analyze-resource-job-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #41은 draft PR 상태를 유지한다.
- #42는 #41 head `4088d62` 기준으로 다음 agent job 생성 stack 확인 대상이다.

### 작업 카드 40-2. #40 자료 분석 작업 생성 API 최신 #37 기준 정리

처리 시각: 2026-06-25 14:11 KST

변경 내용:

- #40 `feature/analyze-resource-job-api` 브랜치에 최신 #37 `feature/room-ai-documents-api` head `4a9e7e3`를 병합했다.
- 병합 충돌은 없었다.
- agent service의 resource 일반 Service 직접 의존을 제거하고 `ResourcePublicService.getReadableResource` 경계로 보정했다.
- agent 패키지는 다른 도메인의 Repository/Entity/일반 Service를 직접 참조하지 않는 상태로 확인했다.
- `PostgresIntegrationTestSupport.java`와 `V1__initial_schema.sql`은 develop 대비 diff가 없는 상태로 확인했다.
- #40 PR 본문을 최신 로컬 검증 결과와 stacked PR checks 없음 사유로 갱신했다.

검증 결과:

- #40: `./gradlew test --tests '*ArchitectureTest'` 통과
- #40: `./gradlew compileTestJava` 통과
- #40: `./gradlew cleanTest test` 통과
- #40: `git diff --check` 통과
- #40: head `e2b7e5c`, base `feature/room-ai-documents-api`, mergeState `CLEAN`
- #40: GitHub checks 없음. base가 `feature/room-ai-documents-api`인 stacked PR이라 check run이 보고되지 않음

메모:

- #40은 draft PR 상태를 유지한다.
- #41은 #40 head `e2b7e5c` 기준으로 다음 agent job 생성 stack 확인 대상이다.
...
- #31: GitHub checks 없음. base가 `feature/resource-basic-foundation`인 stacked PR이라 check run이 보고되지 않음
- 이후 2026-06-25 17:39 KST에 #31 head `3bf14f5`로 한 번 더 최신 #25 base를 병합했다. 현재 상태는 작업 카드 31-3을 기준으로 본다.

메모:

- #31은 현재 draft가 아니며, 최신 상태는 작업 카드 31-3을 기준으로 본다.
- #39는 #31 head `3bf14f5` 기준으로 다음 확인 대상이다.
...
- 이후 2026-06-25 17:45 KST에 #39 head `05721e4`로 최신 #31 base를 다시 병합하고 ready로 전환했다. 현재 상태는 작업 카드 39-3을 기준으로 본다.
...
| #58 | `[feat] 프로젝트룸 수정 결제 종료 API 추가` | `feature/project-room-management-api` | `feature/project-room-members-invitations` | `e471787` | checks 없음, merge clean, ready | #19 위에 프로젝트룸 수정, 계약/입금 수정, 종료 API 추가. develop 병합용으로는 #85 재구성 |
| #85 | `[feat] 프로젝트룸 수정 결제 종료 API 추가` | `codex/develop-project-room-management-api` | `develop` | `c404b02` | `build` pass, review required | 최신 `develop` 위에서 #58 순수 변경분 재구성. develop 병합 검토 후보 |
...
| #30 | `[feat] 타이머 작업시간 기본 API 추가` | `feature/time-log-basic-api` | `feature/work-task-wbs-api` | `f162377` | checks 없음, merge clean | 6/25 기준 time_logs start/pause/resume/stop/heartbeat 기본 API 추가. develop 병합용으로는 #83 재구성 |
| #83 | `[feat] 타이머 작업시간 기본 API 추가` | `codex/develop-time-log-basic-api` | `develop` | `774502c` | `build` pass, review required | 최신 `develop` 위에서 #30 순수 변경분 재구성. develop 병합 검토 후보 |
| #31 | `[feat] 자료 관련 문서 조회 API 추가` | `feature/resource-related-api` | `feature/resource-basic-foundation` | `3bf14f5` | checks 없음, merge clean, ready | 최신 #25 base 재병합 후 CLEAN. 6/25 기준 resource_relations 조회 API 유지 |
...
| #39 | `[feat] 저장 용량 조회 API 추가` | `feature/storage-usage-api` | `feature/resource-related-api` | `05721e4` | checks 없음, merge clean, ready | 최신 #31 base 병합, 공개 서비스 경계 보정, draft 해제 완료 |
...
| #64 | `feat: connect resource upload storage usage` | `feature/resource-upload-storage-usage` | `feature/resource-upload-policy` | `637fa26` | checks 없음, merge clean, ready | 최신 #57 base 병합 후 CLEAN. `StorageUsagePublicService` 공개 경계로 업로드/삭제 사용량 기록과 해제 연결 |
| #67 | `[feat] 자료 삭제 저장소 객체 정리 추가` | `feature/resource-delete-storage-cleanup` | `feature/resource-upload-storage-usage` | `8c553ca` | checks 없음, merge clean, ready | 최신 #64 base 병합 후 CLEAN. 자료 삭제 시 연결 storage object best-effort 삭제 추가 |
...
| #47 | `[feat] 내 프로필 수정 API 추가` | `feature/user-me-update-api` | `develop` | `bef21d7` | `build` pass, review required, ready | 최신 `develop` 병합과 ArchUnit 기준 검증 완료. 6/25 기준 PATCH /api/me 기본 API 추가 |
| #48 | `[feat] 사용자 설정 API 추가` | `feature/user-preferences-api` | `feature/user-me-update-api` | `82a515e` | checks 없음, merge clean, ready | 최신 #47 base 병합과 공개 서비스 권한 경계 보정 완료. 6/25 기준 GET/PATCH /api/me/preferences 추가 |
| #49 | `[feat] 사용자 알림 설정 API 추가` | `feature/user-notification-preferences-api` | `feature/user-preferences-api` | `6f53d4e` | checks 없음, merge clean, ready | 최신 #48 base 병합 완료. 6/25 기준 GET/PATCH /api/me/notification-preferences 추가 |
| #50 | `[feat] 사용자 개인정보 동의 API 추가` | `feature/user-privacy-consents-api` | `feature/user-notification-preferences-api` | `dbfc595` | checks 없음, merge clean, ready | 최신 #49 base 병합 완료. 6/25 기준 GET/PATCH /api/me/privacy-consents 기본 API 골격 추가 |
| #51 | `[feat] 내 프로젝트룸 목록 API 추가` | `feature/user-project-rooms-api` | `feature/user-privacy-consents-api` | `f16f888` | checks 없음, merge clean, ready | 최신 #50 기준 병합과 충돌 해결 완료. 6/25 기준 GET /api/me/project-rooms 추가 |

## Draft PR 후속 전환 메모

2026-06-25 16:13 KST 기준 #47은 ready로 전환했다.
남은 draft PR은 #62다.

draft PR은 폐기 상태가 아니다.
mergeState가 CLEAN이고 로컬 검증/CI 또는 stacked PR checks 없음 사유가 확인된 draft PR은 ready로 전환했다.
#39는 2026-06-25 17:45 KST에 ready로 전환했다. #62는 DIRTY 상태라 draft로 유지한다.
...
| 자료 업로드/삭제 | `POST /api/resources`는 개인 또는 프로젝트룸 자료 업로드, `DELETE /api/resources/{id}`는 자료 삭제 | #25는 파일/S3 업로드 전 단계의 자료 카드 메타데이터 저장/조회 기반, 댓글 API, 파일 메타데이터/버전 API, summary 조회 API까지 구현함. #46에서 download-url API 뼈대를 추가했고, #52에서 S3 presigned URL Provider를 추가함. #54에서 S3 저장/삭제 Service 경계를 추가했고, #55에서 multipart upload를 `resources`, `resource_files`, `resource_versions` v1 생성까지 연결하고 storage 공개 포트는 `StoragePublicService`로 보정함. #56에서 DB 저장 실패 시 StoragePublicService 보상 삭제를 추가했고, #57에서 파일 크기/MIME allow-list 검사를 최신 #56 위에서 재검증함. #64에서 #39 storage usage accounting boundary를 upload stack에 병합하고 업로드/삭제 사용량 기록과 해제를 연결함. #67에서 삭제 시 storage object best-effort 정리를 추가함 |
...
1. 팀원이 `develop` 병합을 계속 진행 중이면, 먼저 #83처럼 최신 `develop` 위에 바로 올릴 수 있는 작은 PR 후보를 찾고 PR 번호를 사용자에게 보고한다.
2. #47, #83, #85는 `develop` 대상, CI 통과, review required 상태이므로 팀 리뷰 후 병합 검토 가능하다.
3. #48은 최신 #47 기준으로 정리됐고, #47 merge 뒤 다음 user/settings stack 후보로 재확인한다.
4. user/settings stack #47~#51은 최신 기준으로 정리됐고, #47 merge 뒤 #48부터 순서대로 재확인한다.
5. 다음 develop 대상 재구성 후보는 #59처럼 base가 이미 develop에 들어간 stack의 다음 작은 PR부터 검토한다.
6. #67은 최신 #64 head `637fa26` 기준 충돌 해소와 로컬 검증이 끝났다.
7. #39는 최신 #31 head `3bf14f5` 기준 충돌 해소와 ready 전환이 끝났다.
8. #67 이후 resource delete stack은 #75부터 최신 #67 head `8c553ca` 기준으로 순차 재검증한다.
9. 남은 draft PR은 #62이며 DIRTY 해소 뒤 ready 전환한다.
10. 다음 추천 작업은 agent 실제 모델 adapter/RAG 연결 전 경계 정리, 6/25 API `.http` 예시 정리, 또는 남은 FK/인덱스 세부 정책 검토다.
11. #19~#81은 6/25 기준으로 계속 재검토하고 차이만 보정한다.
